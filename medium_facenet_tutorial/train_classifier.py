...
def main(input_directory, model_path, classifier_output_path, batch_size, num_threads, num_epochs,
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(log_device_placement=False)) as sess:
        ...
        if is_train:
            images, labels, class_names = _load_images_and_labels(train_set, image_size=200, batch_size=batch_size,
                                                                  num_threads=num_threads, num_epochs=num_epochs,
                                                                  random_flip=True, random_brightness=True,
                                                                  random_contrast=True)
        else:
            images, labels, class_names = _load_images_and_labels(test_set, image_size=200, batch_size=batch_size,
        ...
        if emb_array is None:
            logger.error('No embeddings were created. Check your data and preprocessing steps.')
            return
...
def _get_test_and_train_set(input_dir, min_num_images_per_label, split_ratio=0.7):
    ...
    :return: 
    """
    dataset = lfw_input.get_dataset(input_dir)
    dataset = lfw_input.filter_dataset(dataset, min_images_per_label=min_num_images_per_label)
    train_set, test_set = lfw_input.split_dataset(dataset, split_ratio=split_ratio)
...
def _load_images_and_labels(dataset, image_size, batch_size, num_threads, num_epochs, random_flip=False,
    ...
    print(f"Répartition des labels : {label_counts}")
...
def _create_embeddings(embedding_layer, images, labels, images_placeholder, phase_train_placeholder, sess):
    ...
    try:
        ...
        while True:
            ...
            logger.info(f"Batch images shape: {batch_images.shape}")     # Ajoute ceci
            ...
            # 🔁 Si le nombre d'embeddings est un multiple du nombre de labels, répétons les labels
            if emb.shape[0] > batch_labels.shape[0] and emb.shape[0] % batch_labels.shape[0] == 0:
                repeat_factor = emb.shape[0] // batch_labels.shape[0]
                batch_labels = np.repeat(batch_labels, repeat_factor)
                logger.info(f"Labels repeated {repeat_factor}x to match embeddings shape") 
    ...
    if emb_array is None or label_array is None:
        logger.error('No embeddings or labels to save. Skipping save.')
        return emb_array, label_array
    # 💾 Sauvegarde des embeddings et labels pour analyse
    np.savez('C:/Users/Zeff/reconnaissance_faciale/app/output/embeddings_labels.npz', embeddings=emb_array, labels=label_array)
    logger.info('✅ Embeddings et labels sauvegardés dans C:/Users/Zeff/reconnaissance_faciale/app/output/embeddings_labels.npz')
